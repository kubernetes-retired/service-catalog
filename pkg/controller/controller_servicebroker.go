...
func (c *controller) reconcileServiceBroker(broker *v1beta1.ServiceBroker) error {
	...
	if broker.DeletionTimestamp == nil { // Add or update
		...
		metrics.BrokerServiceClassCount.WithLabelValues(fmt.Sprintf("%s_%s",broker.Name,broker.Namespace)).Set(float64(len(payloadServiceClasses)))
		metrics.BrokerServicePlanCount.WithLabelValues(fmt.Sprintf("%s_%s",broker.Name,broker.Namespace)).Set(float64(len(payloadServicePlans)))
	...
	if finalizers := sets.NewString(broker.Finalizers...); finalizers.Has(v1beta1.FinalizerServiceCatalog) {
		...
		metrics.BrokerServiceClassCount.DeleteLabelValues(fmt.Sprintf("%s_%s",broker.Name,broker.Namespace))
		metrics.BrokerServicePlanCount.DeleteLabelValues(fmt.Sprintf("%s_%s",broker.Name,broker.Namespace))
		return nil
	}
