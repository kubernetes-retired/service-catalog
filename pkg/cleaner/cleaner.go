...
func (c *Cleaner) scaleDownController(namespace, controllerName string) error {
	...
	deployment, err := c.client.AppsV1().Deployments(namespace).Get(context.Background(), controllerName, metav1.GetOptions{})
	switch {
	case err == nil:
	...
	_, err = c.client.AppsV1().Deployments(deploymentCopy.Namespace).Update(context.Background(), deploymentCopy, metav1.UpdateOptions{})
	if err != nil {
...
func (c *Cleaner) removeWebhookConfigurations(names []string) error {
	...
	listMutating, err := c.client.AdmissionregistrationV1beta1().MutatingWebhookConfigurations().List(context.Background(), metav1.ListOptions{})
	if err != nil {
	...
	for _, mwc := range listMutating.Items {
		...
		err = c.client.AdmissionregistrationV1beta1().MutatingWebhookConfigurations().Delete(context.Background(), mwc.Name, metav1.DeleteOptions{})
		if err != nil {
	...
	listValidating, err := c.client.AdmissionregistrationV1beta1().ValidatingWebhookConfigurations().List(context.Background(), metav1.ListOptions{})
	if err != nil {
	...
	for _, vwc := range listValidating.Items {
		...
		err = c.client.AdmissionregistrationV1beta1().ValidatingWebhookConfigurations().Delete(context.Background(), vwc.Name, metav1.DeleteOptions{})
		if err != nil {
...
func (c *Cleaner) removeCRDs(apiextensionsClient apiextensionsclientset.Interface) error {
	...
	list, err := apiextensionsClient.ApiextensionsV1beta1().CustomResourceDefinitions().List(context.Background(), metav1.ListOptions{})
	if err != nil {
	...
	for _, crd := range list.Items {
		...
		err := apiextensionsClient.ApiextensionsV1beta1().CustomResourceDefinitions().Delete(context.Background(), crd.Name, metav1.DeleteOptions{})
		if err != nil {
