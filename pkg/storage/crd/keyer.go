...
//
// It is meant to be passed to a Store's KeyRootFunc field, so that a CRD based storage interface
...
//
// It is meant to be passed to a Store's KeyRoot field, so that a CRD based storage interface
// can parse the namespace and name from fields that it is later given
...
// name for a given etcd-style key. This function is intended to be used in a CRD based
