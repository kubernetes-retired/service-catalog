...
func (x *ParameterSource) CodecEncodeSelf(e *codec1978.Encoder) {
	...
	if x == nil {
		...
		if false {
			...
			_, _, _ = yysep2, yyq2, yy2arr2
			const yyr2 bool = false
			yyq2[0] = x.ConfigMapKeyRef != nil
			yyq2[1] = x.SecretKeyRef != nil
			yyq2[2] = x.Raw != nil
			var yynn2 int
			if yyr2 || yy2arr2 {
				r.EncodeArrayStart(3)
			...
			if yyr2 || yy2arr2 {
				...
				if yyq2[2] {
					if x.Raw == nil {
						...
						_ = yym10
						if false {
						} else if z.HasExtensions() && z.EncExt(x.Raw) {
						} else if !yym10 && z.IsJSONHandle() {
							z.EncJSONMarshal(x.Raw)
						} else {
							z.EncFallback(x.Raw)
						}
					}
				...
				if yyq2[2] {
					z.EncSendContainerState(codecSelfer_containerMapKey1234)
					r.EncodeString(codecSelferC_UTF81234, string("raw"))
					z.EncSendContainerState(codecSelfer_containerMapValue1234)
					if x.Raw == nil {
						r.EncodeNil()
					} else {
	...
	for yyj3 := 0; ; yyj3++ {
		...
		case "configMapKeyRef":
			if r.TryDecodeAsNil() {
				...
				if x.ConfigMapKeyRef == nil {
					x.ConfigMapKeyRef = new(pkg3_v1.ConfigMapKeySelector)
				}
		...
		case "secretKeyRef":
			if r.TryDecodeAsNil() {
				...
				if x.SecretKeyRef == nil {
					x.SecretKeyRef = new(pkg3_v1.SecretKeySelector)
				}
		...
		case "raw":
			if r.TryDecodeAsNil() {
				...
				if x.Raw == nil {
					x.Raw = new(pkg4_runtime.RawExtension)
				}
				yym7 := z.DecBinary()
				_ = yym7
				if false {
				} else if z.HasExtensions() && z.DecExt(x.Raw) {
				} else if !yym7 && z.IsJSONHandle() {
					z.DecJSONUnmarshal(x.Raw)
				} else {
					z.DecFallback(x.Raw, false)
				}
			}
		default:
			z.DecStructFieldNotFound(-1, yys3)
		...
		case "parameters":
			if r.TryDecodeAsNil() {
				...
				if false {
					...
					h.decSliceParameter((*[]Parameter)(yyv8), d)
				}
			}
		case "externalID":
			if r.TryDecodeAsNil() {
...
func (x *InstanceSpec) codecDecodeSelfFromArray(l int, d *codec1978.Decoder) {
	...
	if r.TryDecodeAsNil() {
		...
		if false {
			...
			h.decSliceParameter((*[]Parameter)(yyv17), d)
		}
	}
	yyj12++
	if yyhl12 {
		...
		case "parameters":
			if r.TryDecodeAsNil() {
				...
				if false {
					...
					h.decSliceParameter((*[]Parameter)(yyv5), d)
				}
			}
		case "secretName":
			if r.TryDecodeAsNil() {
...
func (x *BindingSpec) codecDecodeSelfFromArray(l int, d *codec1978.Decoder) {
	...
	if r.TryDecodeAsNil() {
		...
		if false {
			...
			h.decSliceParameter((*[]Parameter)(yyv13), d)
		}
	}
	yyj11++
	if yyhl11 {
