...
func (c *controller) reconcileServiceInstanceDelete(instance *v1alpha1.ServiceInstance) error {
	...
	for _, credentials := range credentialsList {
		if instance.Name == credentials.Spec.ServiceInstanceRef.Name {
			...
			s := fmt.Sprintf(
				`ServiceInstance "%v/%v": Delete instance blocked by existing ServiceBindings associated with this instance.  All credentials must be removed first.`,
				instance.Namespace,
				instance.Name)
			glog.Warning(s)
...
func (c *controller) resolveReferences(instance *v1alpha1.ServiceInstance) (*v1alpha1.ServiceInstance, error) {
	...
	if instance.Spec.ClusterServicePlanRef == nil {
		if sc == nil {
			...
			if scErr != nil {
				return nil, fmt.Errorf(`ServiceInstance "%v/%v": Couldn't find ClusterServiceClass (K8S: %s)": %v`, instance.Namespace, instance.Name, instance.Spec.ClusterServiceClassRef.Name, scErr.Error())
			}
		}
