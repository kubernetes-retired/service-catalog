...
type ClusterServiceClassLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.ClusterServiceClass, err error)
	// Get retrieves the ClusterServiceClass from the index for a given name.
	Get(name string) (*v1alpha1.ClusterServiceClass, error)
...
// List lists all ClusterServiceClasses in the indexer.
func (s *clusterServiceClassLister) List(selector labels.Selector) (ret []*v1alpha1.ClusterServiceClass, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ClusterServiceClass))
	})
	return ret, err
}

// Get retrieves the ClusterServiceClass from the index for a given name.
func (s *clusterServiceClassLister) Get(name string) (*v1alpha1.ClusterServiceClass, error) {
	key := &v1alpha1.ClusterServiceClass{ObjectMeta: v1.ObjectMeta{Name: name}}
	obj, exists, err := s.indexer.Get(key)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("clusterserviceclass"), name)
	}
	return obj.(*v1alpha1.ClusterServiceClass), nil
}
