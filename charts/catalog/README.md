...
| `apiserver.storage.type` | The storage backend to use; valid values are `etcd`, `crd` and `tpr` | `etcd` |
...
| `apiserver.storage.crd.globalNamespace` | If storage type is `crd`: Some service catalog resources are not namespaced, but custom resources must be; setting this designates a namespace that will be treated as a container for such resources | `servicecatalog` |
