...
func TestNamespacedStorageSetDelete(t *testing.T) {
	...
	if nil != storage.Get("ns", "tipe", "name") {
		t.Fatal("Expected no results from an empty storage")
	}

	storage.Set("ns", "tipe", "name", &servicecatalog.Broker{})
	if nil == storage.Get("ns", "tipe", "name") {
...
func TestNamespacedStorageGetList(t *testing.T) {
	storage := make(NamespacedStorage)
	storage.Set("ns1", "type1", "broker1", &servicecatalog.Broker{})
	storage.Set("ns1", "type1", "broker2", &servicecatalog.Broker{})
	storage.Set("ns1", "type1", "broker3", &servicecatalog.Broker{})

	storage.Set("ns2", "type1", "broker1", &servicecatalog.Broker{})
	storage.Set("ns2", "type1", "broker2", &servicecatalog.Broker{})

	objects := storage.GetList("ns1", "type1")
...
func TestResponseWriterGetResponse(t *testing.T) {
	...
	if response := rw.getResponse(); response.StatusCode != 200 {
