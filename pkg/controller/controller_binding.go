...
func (c *controller) setAndUpdateOrphanMitigation(binding *v1alpha1.ServiceBinding, toUpdate *v1alpha1.ServiceBinding, instance *v1alpha1.ServiceInstance, serviceClass *v1alpha1.ClusterServiceClass, brokerName string, errorStr string) error {
	s := fmt.Sprintf(
		"%s: Starting orphan mitgation for ServiceBinding \"%s/%s\" for ServiceInstance \"%s/%s\" of ClusterServiceClass %q at ServiceBroker %q, %v",
		typez, binding.Name, binding.Namespace, instance.Namespace, instance.Name, serviceClass.Name, brokerName, errorStr,
	)
...
func (c *controller) reconcileServiceBinding(binding *v1alpha1.ServiceBinding) error {
	...
	glog.V(4).Infof(
		"%s: Processing ServiceBinding %v/%v",
		typez, binding.Namespace, binding.Name,
	)
	...
	if err != nil {
		s := fmt.Sprintf(
			"%s: ServiceBinding \"%s/%s\" references a non-existent ServiceInstance \"%s/%s\"",
	...
	if !isPlanBindable(serviceClass, servicePlan) {
		s := fmt.Sprintf(
			...
			typez, binding.Namespace, binding.Name, instance.Spec.ExternalClusterServiceClassName, instance.Spec.ExternalClusterServicePlanName,
		)
		glog.Warning(s)
	...
	if binding.DeletionTimestamp == nil && !binding.Status.OrphanMitigationInProgress { // Add or update
		glog.V(4).Infof(
			"%s: Adding/Updating ServiceBinding %v/%v",
			typez, binding.Namespace, binding.Name,
		)
		...
		if err != nil {
			s := fmt.Sprintf(
				"%s: Failed to get namespace %q during binding: %s",
				typez, instance.Namespace, err,
			)
			glog.Info(s)
		...
		if !isServiceInstanceReady(instance) {
			s := fmt.Sprintf(
				`%s: ServiceBinding cannot begin because referenced instance "%v/%v" is not ready`,
				typez, instance.Namespace, instance.Name,
			)
			glog.Info(s)
		...
		if binding.Spec.Parameters != nil || binding.Spec.ParametersFrom != nil {
			...
			if err != nil {
				s := fmt.Sprintf(
					"%s: Failed to prepare ServiceBinding parameters\n%s\n %s",
					typez, binding.Spec.Parameters, err,
				)
				glog.Warning(s)
			...
			if err != nil {
				s := fmt.Sprintf(
					`%s: Failed to generate the parameters checksum to store in the Status of ServiceBinding "%s/%s": %s`,
			...
			if err != nil {
				s := fmt.Sprintf(
					`%s: Failed to marshal the parameters to store in the Status of ServiceBinding "%s/%s": %s`,
					typez, binding.Namespace, binding.Name, err,
				)
				glog.Info(s)
				c.recorder.Eventf(binding, corev1.EventTypeWarning, errorWithParameters, s)
		...
		if utilfeature.DefaultFeatureGate.Enabled(scfeatures.OriginatingIdentity) {
			...
			if err != nil {
				s := fmt.Sprintf(
					`%s: Error building originating identity headers for binding ServiceBinding "%v/%v": %v`,
					typez, binding.Namespace, binding.Name, err,
				)
				glog.Warning(s)
		...
		if netErr, ok := err.(net.Error); ok && netErr.Timeout() {
			...
			if httpErr, ok := osb.IsHTTPError(err); ok {
				...
				s := fmt.Sprintf(
					"%s: Error creating ServiceBinding \"%s/%s\" for ServiceInstance \"%s/%s\" of ClusterServiceClass %q at ClusterServiceBroker %q, %v",
					typez, binding.Name, binding.Namespace, instance.Namespace, instance.Name, serviceClass.Spec.ExternalName, brokerName, httpErr.Error(),
				)
				glog.Warning(s)
			...
			s := fmt.Sprintf(
				"%s: Error creating ServiceBinding \"%s/%s\" for ServiceInstance \"%s/%s\" of ClusterServiceClass %q at ClusterServiceBroker %q: %s",
				typez, binding.Name, binding.Namespace, instance.Namespace, instance.Name, serviceClass.Spec.ExternalName, brokerName, err,
			)
			glog.Warning(s)
			c.recorder.Event(binding, corev1.EventTypeWarning, errorBindCallReason, s)
			c.setServiceBindingCondition(
			...
			if !time.Now().Before(toUpdate.Status.OperationStartTime.Time.Add(c.reconciliationRetryDuration)) {
				s := fmt.Sprintf(
					`%s: Stopping reconciliation retries on ServiceBinding "%v/%v" because too much time has elapsed`,
					typez, binding.Namespace, binding.Name,
				)
				glog.Info(s)
		...
		if err != nil {
			s := fmt.Sprintf(
				"%s: Error injecting binding results for ServiceBinding \"%s/%s\": %s",
				typez, binding.Namespace, binding.Name, err,
			)
			glog.Warning(s)
		...
		glog.V(5).Infof(
			"%s: Successfully bound to ServiceInstance %v/%v of ClusterServiceClass %v at ClusterServiceBroker %v",
			typez, instance.Namespace, instance.Name, serviceClass.Name, brokerName,
		)
	...
	if finalizers := sets.NewString(binding.Finalizers...); finalizers.Has(v1alpha1.FinalizerServiceCatalog) || binding.Status.OrphanMitigationInProgress {
		...
		if utilfeature.DefaultFeatureGate.Enabled(scfeatures.OriginatingIdentity) {
			...
			if err != nil {
				s := fmt.Sprintf(
					`%s: Error building originating identity headers for unbinding ServiceBinding "%v/%v": %v`,
					typez, binding.Namespace, binding.Name, err,
				)
				glog.Warning(s)
				c.recorder.Event(binding, corev1.EventTypeWarning, errorWithOriginatingIdentity, s)
		...
		if err != nil {
			...
			if !time.Now().Before(toUpdate.Status.OperationStartTime.Time.Add(c.reconciliationRetryDuration)) {
				...
				if toUpdate.Status.OrphanMitigationInProgress {
					s := fmt.Sprintf(
						`%s: Stopping reconciliation retries on ServiceBinding "%v/%v" because too much time has elapsed during orphan mitigation`,
						typez, binding.Namespace, binding.Name,
					)
					glog.Info(s)
					c.recorder.Event(binding, corev1.EventTypeWarning, errorReconciliationRetryTimeoutReason, s)
		...
		if toUpdate.Status.OrphanMitigationInProgress {
			s := fmt.Sprintf(
				`%s: Orphan mitigation successful for ServiceBinding "%v/%v"`,
				typez, binding.Namespace, binding.Name,
			)
			c.setServiceBindingCondition(toUpdate,
				v1alpha1.ServiceBindingConditionReady,
		...
		glog.V(5).Infof(
			"%s: Successfully deleted ServiceBinding %v/%v of ServiceInstance %v/%v of ClusterServiceClass %v at ClusterServiceBroker %v",
			typez, binding.Namespace, binding.Name, instance.Namespace, instance.Name, serviceClass.Name, brokerName,
		)
	}
	return nil
}
...
func (c *controller) injectServiceBinding(binding *v1alpha1.ServiceBinding, credentials map[string]interface{}) error {
	glog.V(5).Infof(
		"%s: Creating/updating Secret %v/%v",
...
func setServiceBindingConditionInternal(toUpdate *v1alpha1.ServiceBinding,
	...
	glog.V(5).Infof(
		"%s: Setting ServiceBinding '%v/%v' condition %q to %v",
	...
	if len(toUpdate.Status.Conditions) == 0 {
		glog.Infof(
			`%s: Setting lastTransitionTime for ServiceBinding "%v/%v" condition %q to %v`,
	...
	for i, cond := range toUpdate.Status.Conditions {
		if cond.Type == conditionType {
			if cond.Status != newCondition.Status {
				glog.V(3).Infof(
					`%s: Found status change for ServiceBinding "%v/%v" condition %q: %q -> %q; setting lastTransitionTime to %v`,
	...
	glog.V(3).Infof(
		"Setting lastTransitionTime for ServiceBinding '%v/%v' condition %q to %v",
...
func (c *controller) updateServiceBindingStatus(toUpdate *v1alpha1.ServiceBinding) (*v1alpha1.ServiceBinding, error) {
	glog.V(4).Infof(
		"%s: Updating status for ServiceBinding %v/%v",
	...
	if err != nil {
		glog.Errorf(
			"%s: Error updating status for ServiceBinding %v/%v",
			typez, toUpdate.Namespace, toUpdate.Name,
		)
	}
...
func (c *controller) updateServiceBindingCondition(
	...
	glog.V(4).Infof(
		"%s: Updating %v condition for ServiceBinding %v/%v to %v (Reason: %q, Message: %q)",
		typez, conditionType, binding.Namespace, binding.Name, status, reason, message,
	)
	_, err = c.serviceCatalogClient.ServiceBindings(binding.Namespace).UpdateStatus(toUpdate)
	if err != nil {
		glog.Errorf(
			"%s: Error updating %v condition for ServiceBinding %v/%v to %v: %v",
			typez, conditionType, binding.Namespace, binding.Name, status, err,
		)
	}
	return err
}
...
func (c *controller) bindingDelete(obj interface{}) {
	...
	glog.V(4).Infof(
		"%s: Received delete event for ServiceBinding %v/%v; no further processing will occur",
		typez, binding.Namespace, binding.Name,
	)
}
